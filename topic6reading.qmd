...
1) **Honest commitment**: The central bank announces $\pi=0$, the public believes it, and the bank actually delivers $\pi=0$. So expected and actual inflation are both zero. Because there is no inflation surprise, output remains at $Y^*$. The only remaining problem is that the policymaker still prefers $Y^T=Y^*+\lambda$, so there is an output-target gap of size $\lambda$. Loss is therefore
$$L^* = (b/2)\lambda^2.$$

2) **Discretionary equilibrium**: The public understands the bank's incentive to create surprise inflation, so it expects the time-consistent inflation rate
$$E_{-1}\pi=\pi=\frac{b\kappa\lambda}{a}.$$
Because actual inflation equals expected inflation, there is again no surprise and output remains at $Y^*$. The difference from honest commitment is that inflation is now positive, so the bank suffers both the inflation cost and the same output-target gap as before. Loss is
$$L^e = (b/2)\lambda^2(1 + (b\kappa^2)/a).$$

3) **Cheating**: Suppose the public expects $\pi=0$, but after expectations are fixed the central bank re-optimizes and chooses positive inflation. Now actual inflation exceeds expected inflation, so output temporarily rises above $Y^*$. This reduces the gap between actual output and the policymaker's preferred target $Y^T$, which is why cheating is tempting. The cheating loss is
$$L^c = (a/(a+b\kappa^2))(b/2)\lambda^2,$$
and this is lower than $L^*$, so once low expectations are in place the policymaker would like to deviate.

4) **Purist strategy**: The public expects the discretionary inflation rate, but the central bank unexpectedly sets $\pi=0$. Now actual inflation is below expected inflation, so output falls below $Y^*$. Since the policymaker already wanted output above $Y^*$, this makes the output-target gap even worse. This is the worst case:
$$L^p = (b/2)\lambda^2(1+(b\kappa^2)/a)^2 > L^e.$$
**Key insight**: sudden disinflation without prior credibility can create a recession before credibility has time to improve expectations.

It helps to summarize the four cases in plain language:

- honest commitment: low expected inflation, low actual inflation, no recession, but the policymaker still cannot achieve $Y^T$
- discretionary equilibrium: high expected inflation, high actual inflation, no output gain, so the economy gets inflation without benefit
- cheating: low expected inflation, higher actual inflation, temporary boom, which explains the temptation to cheat
- purist strategy: high expected inflation, low actual inflation, temporary recession, which explains why disinflation is costly without credibility
