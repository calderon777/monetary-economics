...
1) **Unexpected policy innovations (surprises).** In rational expectations models, monetary policy affects output mainly when it is not expected. If households, firms and workers can already see a policy change coming, they adjust their behaviour in advance, so the real effects are much smaller. In Lucas's analysis, this means that only the unexpected part of policy can temporarily raise output; systematic, predictable policy cannot do so in a reliable way.

2) **Nominal rigidities.** Monetary policy can also have real effects because wages and prices do not adjust instantly. If firms change prices slowly, an interest-rate change can affect borrowing, spending and production before the full price adjustment has taken place. This is why modern central banks emphasise sticky wages and prices when explaining how policy is transmitted to the real economy.
