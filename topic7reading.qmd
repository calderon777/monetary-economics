...
fetch_boe_bank_rate <- function() {
  url <- paste0(
    ...
    "CSVF=TT&DAT=RNG&FD=1&FM=Jan&FNY=Y&FY=2000&",
    "TD=31&TM=Dec&TY=", year(Sys.Date()),
    "&SeriesCodes=IUMBEDR&UsingCodes=Y&VPD=Y"
  )
  f <- download_with_cache(url, "data_cache/boe_rate_monthly.csv")
  raw <- readr::read_lines(f)
  start_idx <- which(str_detect(raw, "^(Date|DATE),"))
  if (length(start_idx) == 0) stop("Could not locate Bank Rate table in BoE download.")
  df <- readr::read_csv(
    I(raw[start_idx:length(raw)]),
...
#| label: fig-qe
#| cache: false
#| fig-cap: "Selected large-scale asset purchase announcements for the BoE, ECB, and Fed, showing how announced programme sizes were expanded over time"
#| fig-alt: "Step chart of selected asset purchase announcement sizes by central bank"
#| fig-width: 9
#| fig-height: 7
...
qe_steps <- tribble(
  ...
  "BoE", "APF/QE authorised size", as.Date("2011-10-06"), 275,  "GBP bn",
  "BoE", "APF/QE authorised size", as.Date("2012-02-09"), 325,  "GBP bn",
  "BoE", "APF/QE authorised size", as.Date("2012-07-05"), 375,  "GBP bn",
  "BoE", "APF/QE authorised size", as.Date("2016-08-04"), 435,  "GBP bn",
  ...
  "Fed", "QE2 announced totals",   as.Date("2010-11-03"), 600,  "USD bn",
  "Fed", "QE3 announced pace",     as.Date("2012-09-13"), 40,   "USD bn/month",
  "Fed", "QE3 announced pace",     as.Date("2012-12-12"), 85,   "USD bn/month"
)

ggplot(qe_steps, aes(x = date, y = cum_amount, group = program)) +
  geom_step(linewidth = 0.9, direction = "hv") +
  geom_point(size = 2) +
  facet_wrap(vars(bank, unit), scales = "free", ncol = 1) +
  scale_x_date(date_breaks = "2 years", date_labels = "%Y") +
  labs(
    title = "Selected QE / asset purchase announcements",
    subtitle = "Programme sizes or monthly purchase paces, depending on the scheme",
