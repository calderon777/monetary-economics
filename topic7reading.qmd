...
required_pkgs <- c(
  ...
  "stringr", "janitor", "zoo", "ggrepel", "rvest"
)
...
# Helper functions
download_with_cache <- function(url, destfile, refresh_days = 7) {
  needs_refresh <- !file.exists(destfile)
  if (!needs_refresh) {
    file_age_days <- as.numeric(difftime(Sys.time(), file.info(destfile)$mtime, units = "days"))
    needs_refresh <- is.na(file_age_days) || file_age_days > refresh_days
  }
  if (needs_refresh) {
...
# Policy-rate data from official central-bank/FRED sources
fetch_boe_bank_rate <- function() {
  url <- "https://www.bankofengland.co.uk/boeapps/database/Bank-Rate.asp?os=i"
  page <- read_html(url)
  tbl <- page %>%
    html_table(fill = TRUE) %>%
    purrr::keep(~ all(c("Date Changed", "Rate") %in% names(.x))) %>%
    purrr::pluck(1)
  if (is.null(tbl)) stop("Could not locate Bank Rate history table on the BoE page.")

  changes <- tbl %>%
    transmute(
      date = dmy(`Date Changed`),
      value = readr::parse_number(as.character(Rate))
    ) %>%
    filter(!is.na(date), !is.na(value)) %>%
    arrange(date)

  tibble(date = seq(min(changes$date), Sys.Date(), by = "day")) %>%
...
fetch_fed_target_midpoint <- function() {
  url_t <- "https://fred.stlouisfed.org/graph/fredgraph.csv?id=DFEDTAR"
...
Modern central banks state this in policy‑framework documents. For example, the Federal Reserve's strategy statement explicitly notes that "the inflation rate over the longer run is primarily determined by monetary policy," and therefore the committee can specify a longer‑run inflation goal [@frb2025LongerRunGoalsStatement].
...
anchors <- tribble(
  ...
  "Monetary targeting", "Works well at the effective lower bound (ELB)?", "Medium",
  ...
  "Exchange-rate peg", "Works well at the effective lower bound (ELB)?", "Medium",
  ...
  "Inflation targeting", "Works well at the effective lower bound (ELB)?", "High"
) %>%
...
### Why monetary targeting proved difficult in practice
...
The classic case for exchange-rate pegs stresses two potential benefits [@calvoKumhofCelasun2002NominalExchangeRateAnchoring; @barroGordon1983RulesDiscretionReputation]:
...
nodes <- tribble(
  ~id, ~label, ~x, ~y, ~lx, ~ly,
  "i",   "Policy rate (i)",              0.1, 0.8,  0.1,   0.8,
  "mr",  "Market rates & credit",        0.4, 0.8,  0.4,   0.81,
  "ap",  "Asset prices",                 0.4, 0.6,  0.4,   0.595,
  "fx",  "Exchange rate",                0.4, 0.4,  0.4,   0.39,
  "ad",  "Spending / AD",                0.7, 0.6,  0.705, 0.595,
  "og",  "Output gap",                   0.85,0.6,  0.855, 0.6,
  "inf", "Wages & prices -> inflation", 0.85,0.4,  0.85,  0.39
)
...
#| label: fig-qe
#| cache: false
#| fig-cap: "Selected fixed-size asset purchase announcements for the BoE, ECB, and Fed, shown as event markers rather than a continuous cumulative series"
#| fig-alt: "Faceted event chart of selected QE announcement sizes by central bank"
#| fig-width: 9
#| fig-height: 7

qe_events <- tribble(
  ~bank, ~program, ~date, ~amount, ~unit, ~label,
  "BoE", "APF",   as.Date("2009-03-05"),   75, "GBP bn", "APF 75",
  "BoE", "APF",   as.Date("2009-05-07"),  125, "GBP bn", "125",
  "BoE", "APF",   as.Date("2009-08-06"),  175, "GBP bn", "175",
  "BoE", "APF",   as.Date("2009-11-05"),  200, "GBP bn", "200",
  "BoE", "APF",   as.Date("2011-10-06"),  275, "GBP bn", "275",
  "BoE", "APF",   as.Date("2012-02-09"),  325, "GBP bn", "325",
  "BoE", "APF",   as.Date("2012-07-05"),  375, "GBP bn", "375",
  "BoE", "APF",   as.Date("2016-08-04"),  435, "GBP bn", "435",
  "BoE", "APF",   as.Date("2020-03-19"),  645, "GBP bn", "645",
  "BoE", "APF",   as.Date("2020-06-18"),  745, "GBP bn", "745",
  "BoE", "APF",   as.Date("2020-11-05"),  895, "GBP bn", "895",
  "ECB", "PEPP",  as.Date("2020-03-18"),  750, "EUR bn", "PEPP 750",
  "ECB", "PEPP",  as.Date("2020-06-04"), 1350, "EUR bn", "1350",
  "ECB", "PEPP",  as.Date("2020-12-10"), 1850, "EUR bn", "1850",
  "Fed", "LSAP1", as.Date("2008-11-25"),  600, "USD bn", "LSAP1 600",
  "Fed", "LSAP1", as.Date("2009-03-18"), 1750, "USD bn", "1750",
  "Fed", "QE2",   as.Date("2010-11-03"),  600, "USD bn", "QE2 600"
) %>%
  mutate(panel = paste0(bank, " (", unit, ")"))

ggplot(qe_events, aes(x = date, y = amount)) +
  geom_segment(aes(xend = date, y = 0, yend = amount), linewidth = 0.6, colour = "grey55") +
  geom_point(size = 2.3) +
  geom_text_repel(
    aes(label = label),
    size = 3,
  ...
  labs(
    title = "Selected QE / asset purchase announcements",
    subtitle = "Fixed-size announcements only; open-ended monthly purchase paces are omitted for comparability",
    x = NULL, y = "Announcement size"
  ) +
  theme_minimal(base_size = 12)
```
