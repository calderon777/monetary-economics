...
# All data from FRED for reliability
fetch_boe_bank_rate <- function() {
  url <- "https://fred.stlouisfed.org/graph/fredgraph.csv?id=BOERUKQ"
  f <- download_with_cache(url, "data_cache/boe_rate_fred.csv")
...
fetch_ecb_deposit_facility <- function() {
  url <- "https://fred.stlouisfed.org/graph/fredgraph.csv?id=ECBDFR"
  f <- download_with_cache(url, "data_cache/ecb_rate_fred.csv")
  df <- readr::read_csv(f, show_col_types = FALSE)
  names(df) <- c("date", "value")
  df %>%
    mutate(date = as.Date(date),
...
ggplot(policy_rates, aes(x = date, y = value, group = bank)) +
  geom_line(linewidth = 0.8) +
  geom_vline(data = events, aes(xintercept = date), linetype = "dashed", linewidth = 0.4, inherit.aes = FALSE) +
  geom_text(
    data = events,
