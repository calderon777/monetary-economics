...
<style>
/* Quiz UI */
.quiz-controls { display: flex; gap: 0.75rem; flex-wrap: wrap; align-items: center; margin: 1rem 0; }
.quiz-controls input[type="text"] { padding: 0.5rem 0.6rem; min-width: 240px; }
.quiz-controls button { padding: 0.55rem 0.8rem; }
#quiz-result { margin: 0.5rem 0 1rem 0; font-weight: 600; }
.mcq-key { display: none; } /* keep answer key hidden in the page source UI */

/* Post-submit feedback */
label.is-correct { outline: 2px solid currentColor; border-radius: 6px; padding: 0.15rem 0.25rem; }
label.is-incorrect { outline: 2px dashed currentColor; border-radius: 6px; padding: 0.15rem 0.25rem; }
.quiz-feedback-line { margin-top: 0.35rem; font-size: 0.95rem; }
.quiz-feedback-line.hidden { display: none; }

/* Leaderboard */
#leaderboard { margin-top: 0.5rem; }
#leaderboard table { border-collapse: collapse; width: 100%; }
#leaderboard th, #leaderboard td { border-bottom: 1px solid #ddd; padding: 0.5rem 0.4rem; text-align: left; }
</style>
...
(function () {
  ...
  if (!ENDPOINT || ENDPOINT.includes("PASTE_YOUR_")) {
    container.innerHTML = "<table><tr><td>Leaderboard is not configured yet. Add your Apps Script URL in the page source.</td></tr></table>";
    return;
  }
  ...
  try {
    ...
    if (leaderboard.length === 0) {
      container.innerHTML = "<table><tr><td>No attempts recorded yet.</td></tr></table>";
      return;
    }

    let html = "<table>";
    html += "<tr><th>Rank</th><th>Nickname</th><th>Score</th></tr>";
    html += leaderboard.map((r, i) =>
      "<tr><td>" + (i + 1) + "</td><td>" + r.nickname + "</td><td>" + r.score + " / " + r.total + "</td></tr>"
    ).join("");
    html += "</table>";

    container.innerHTML = html;
  } catch (err) {
    container.innerHTML = "<table><tr><td>Could not load leaderboard.</td></tr></table>";
